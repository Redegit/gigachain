...
class TagMeAgentTracer(AsyncCallbackHandler):
    ...
    async def flush(self, reason: str = "manual") -> None:
        ...
        with self._lock:
            ...
            if self.functions:
                logger_level_func(
                    "Sending %d function definitions (thread_id=%s)", len(self.functions), self._thread_id or "N/A"
                )
                await self.client.send_functions(self.functions)

            if self.dialog:
                logger_level_func("Sending %d chat messages (thread_id=%s)", len(self.dialog), self._thread_id or "N/A")
                metadata = {**self.metadata, "flush_reason": reason, "ts": time.time()}
                await self.client.send_dialog(DialogData(self.dialog, metadata))
    ...
    async def on_chat_model_start(
        ...
        tid = _thread_id_from_kw(**kw)
        with self._lock:
            if self._thread_id is not None and self._thread_id != tid:
            ...
            self.functions = _extract_functions(inv)
            batch = messages[0] if messages else []
            self.dialog = _batch_msgs_to_chat(batch)
